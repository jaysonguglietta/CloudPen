...
| SIEM delivery | Privacy-minimized security events are sent to an exact HTTPS `/v1/events` endpoint; failures enter a D1 outbox with digest, bounded retries, and backoff | Bundle checks and request telemetry tests; production endpoint test remains operational evidence |
...
2. Configure an independently administered SIEM `/v1/events` collector; prove delivery, outbox retry, alert routing, immutable retention, and credential rotation.
