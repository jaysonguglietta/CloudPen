...
export default function Home() {
  ...
  useEffect(() => {
    const saved = window.localStorage.getItem("cloudpen-runs");
    if (saved) {
      try {
        const restoredRuns = JSON.parse(saved) as ValidationRun[];
        window.setTimeout(() => setRuns(restoredRuns), 0);
      } catch {
        window.localStorage.removeItem("cloudpen-runs");
      }
    }
  }, []);

  useEffect(() => {
    window.localStorage.setItem("cloudpen-runs", JSON.stringify(runs));
  }, [runs]);
  ...
  function startValidation() {
    ...
    const chosen = attackPaths.find((path) => path.id === runPathId) ?? attackPaths[0];
    const id = `RUN-${String(300 + runs.length).padStart(4, "0")}`;
    const nextRun: ValidationRun = {
      id,
      name: `${chosen.id} · ${chosen.title}`,
      mode: validationMode,
      status: "Running",
      pathCount: 1,
      findings: 0,
    ...
    setModal(null);
    setView("runs");
    setToast("Validation started inside approved guardrails.");
    window.setTimeout(() => {
      setRuns((current) =>
        current.map((run) =>
          run.id === id
            ? { ...run, status: "Completed", findings: chosen.status === "Mitigated" ? 0 : 1, duration: "2m 14s" }
            : run,
        ),
      );
    }, 2200);
  ...
  function connectAccount(event: React.FormEvent) {
    ...
    setAccountError("");
    setModal(null);
    setToast(`${accountForm.name} is ready for role verification.`);
    setAccountForm({ name: "", id: "", externalId: "" });
  }

  function exportEvidence(path: AttackPath) {
    const report = {
    ...
    setToast("Evidence package exported with sensitive values redacted.");
  }
      ...
      <section className="path-workspace">
        ...
        <article className="panel path-detail">
          ...
          <div className="detail-actions">
            <button className="button primary" onClick={() => openValidation(selectedPath.id)}>▶ Validate path</button>
    ...
    return (
      <section className="panel resource-panel">
        <div className="resource-toolbar">
          ...
          <button className="button secondary" onClick={() => setToast("Inventory sync queued for all healthy connectors.")}>↻ Sync inventory</button>
        </div>
    ...
    return (
      <section className="panel resource-panel">
        <div className="resource-toolbar">
          ...
          <button className="button primary" onClick={() => openValidation()}>＋ New validation</button>
        </div>
          ...
          {runs.map((run) => (
            <article className="run-card" key={run.id}>
              ...
              <div className="run-content">
                ...
                {run.status === "Running" && <div className="run-progress"><span /><p>Collecting CloudTrail evidence and verifying cleanup…</p><button onClick={() => { setRuns((current) => current.map((item) => item.id === run.id ? { ...item, status: "Stopped", duration: "Stopped by operator" } : item)); setToast("Emergency stop requested. Cleanup remains active."); }}>Emergency stop</button></div>}
              </div>
            </article>
          ))}
        </div>
  ...
  function renderGuardrails() {
    const toggle = (key: keyof typeof guardrails) => {
      setGuardrails((current) => ({ ...current, [key]: !current[key] }));
      setToast("Guardrail policy updated for future test plans.");
    };
    return (
      <section className="settings-layout">
          ...
          <div className="settings-list">
            {[
              ...
              <div className="setting-row" key={key}>
                ...
                <button className={`switch ${guardrails[key as keyof typeof guardrails] ? "on" : ""}`} role="switch" aria-checked={guardrails[key as keyof typeof guardrails]} aria-label={label} onClick={() => toggle(key as keyof typeof guardrails)}><span /></button>
              </div>
            ))}
          </div>
        </div>
        <aside className="panel policy-summary">
          ...
          <button className="button secondary full" onClick={() => setToast("Policy JSON exported for review.")}>Export policy</button>
        </aside>
      </section>
    );
  }
    ...
    <div className="app-shell">
      <aside className={`sidebar ${mobileNavOpen ? "open" : ""}`}>
        ...
        <div className="sidebar-bottom">
          <div className="runner-health"><div><span className="pulse" /><strong>Runner connected</strong></div><p>v0.8.4 · us-east-1</p></div>
      ...
      <div className="main-shell">
        <header className="topbar">
          ...
          <div className="topbar-actions">
            ...
            <div className="popover-anchor">
              <button className="avatar-button" aria-label="Open profile menu" onClick={() => setProfileOpen((open) => !open)}>MC</button>
              {profileOpen && <div className="popover profile-popover"><strong>Maya Chen</strong><span>Security administrator</span><button onClick={() => setToast("Profile preferences are saved locally in this prototype.")}>Profile settings</button><button onClick={() => setToast("Demo session retained.")}>Sign out</button></div>}
            </div>
          </div>
        </header>
        ...
        <main>
          <div className="page-heading">
            ...
            <div className="heading-actions"><button className="button secondary" onClick={() => setModal("connect")}>＋ Connect account</button><button className="button primary" onClick={() => openValidation()}>▶ New validation</button></div>
          </div>
      ...
      {modal && <div className="modal-backdrop" role="presentation" onMouseDown={(event) => { if (event.currentTarget === event.target) setModal(null); }}>
        <section className="modal" role="dialog" aria-modal="true" aria-labelledby="modal-title">
          ...
          {modal === "validate" && <>
            ...
            <div className="modal-actions"><button className="button secondary" onClick={() => setModal(null)}>Cancel</button><button className="button primary" disabled={validationMode === "Active canary" && !acknowledged} onClick={startValidation}>Create & run validation</button></div>
          </>}
